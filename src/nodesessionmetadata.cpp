...
session_id_t NodeSessionMetadata::getShardId(const std::set<std::string>& collapsedNames) const {
    ...
    SPDLOG_ERROR("sessionsLebels.size(): {}, size-1: {}", sessionsLevels.size(), sessionsLevels.size() - 1);
    ...
    for (size_t i = 0; i < collapsedNames.size(); ++i) {
        ...
        SPDLOG_LOGGER_DEBUG(dag_executor_logger, "shardId: {}, multiplyFactor: {}, subsessionId: {}, sessionSize: {}", shardId, multiplyFactor, id, sessionSize);
    }
    return shardId;
}
}  // namespace ovms

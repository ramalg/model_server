...
Status Node::setInputs(const Node& dependency, BlobMap& inputs, NodeSessionMetadata& metadata) {
    ...
    const session_id_t shardId = metadata.getShardId(gatherFrom.value_or(std::move(emptySet)));
...
NodeSession& Node::getNodeSession(const NodeSessionMetadata& metadata) {
    ...
    std::optional<uint16_t> expectedEachInputsCount;
    if (gatherFrom) {
        expectedEachInputsCount = 0;
        for (auto nodeName : gatherFrom.value()) {
            expectedEachInputsCount.value() += metadata.getSubsessionSize(nodeName);
        }
    }

...
TEST_F(GatherNodeTest, FullFlowGatherInNonExitNode) {
    ...
    auto subsessions = meta.generateSubsessions(demultiplexerNodeName, 2);
    ...
    EXPECT_EQ(gatheredBlob->size(), nodeRawResults1.size() * 2);
    std::vector<float> resultBlobData(nodeRawResults1.size() * 2);
